...
const addReq = async(req,res)=>{
const {email,title,description,location,name,phoneNumber,wishListed,accepted,category}=req.body;
...
const getAllRequests = async (req, res) => {
    try {
        const {userId} = req.body; // Using query params instead of body
        console.log( userId);
...
const deleteReq = async (req, res) => {
  ...
  const getAllUsersNonAcceptedReq = async (req, res) => {
    try {
        ...
        const posts=await Request.find({accepted:false}).sort({createdAt:-1});
...
export  {addReq,getAllRequests,updateReq,deleteReq,getAllUsersNonAcceptedReq,getAllUsersAcceptedReq,getAllAcceptedReq,getAllNonAcceptedReq};